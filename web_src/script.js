...
const CSV2_FIELDS = [
    ...
    "ch2GapLast",   // CH2 RPM read gap, most recent (ms)
    "ch2GapWorst",  // CH2 RPM read gap, worst since reset (ms)
    "csv2BuildLast",   // CSV2 build (snprintf) time, previous cycle (µs)
    "csv2BuildWorst",  // CSV2 build time, worst since reset (µs)
    "csv2SendLast",    // CSV2 send (events.send) time, previous cycle (µs)
...
// iExcessLive: live Group 3 (iExcess) detector sparkline. Per CSV1 frame the firmware ships the
...
function drawIExcessSpark(c) {
    ...
    const pad = 2;
    let dmax = 0; for (let i = 0; i < N; i++) dmax = Math.max(dmax, S.peak[i], S.minE[i]);
    // Fixed: 0→ceiling so both clamps are framed. Auto: anchor the top to the data (which includes E),
    // so it can't zoom tighter than "E with headroom" — avoids the excessive-zoom-on-a-flat-signal trap.
    const ymax = auto ? Math.max(dmax, 1) * 1.15 : Math.max(ceil, dmax) * 1.05;
    const X = i => pad + i * (w - 2 * pad) / (N - 1), Y = v => h - pad - (Math.min(v, ymax) / ymax) * (h - 2 * pad);
    // floor always; ceiling whenever it's within the visible range
    g.strokeStyle = '#bbb'; g.lineWidth = 1; g.setLineDash([2, 3]);
    g.beginPath(); g.moveTo(0, Y(floor)); g.lineTo(w, Y(floor)); g.stroke();
    if (ceil <= ymax) {
        g.strokeStyle = '#9b59b6'; g.setLineDash([5, 3]);
    ...
    for (let i = 0; i < N; i++) {
        if (S.fired[i]) { const x = X(i); g.beginPath(); g.moveTo(x, 0); g.lineTo(x, h); g.stroke(); }
    }
    // min-E line (amber)
    g.beginPath(); for (let i = 0; i < N; i++) { const x = X(i), y = Y(S.minE[i]); i ? g.lineTo(x, y) : g.moveTo(x, y); }
    g.strokeStyle = '#d68910'; g.lineWidth = 1.4; g.stroke();
    // peak-excess line (teal throughout — the fire is shown by the vertical marker)
    g.beginPath(); for (let i = 0; i < N; i++) { const x = X(i), y = Y(S.peak[i]); i ? g.lineTo(x, y) : g.moveTo(x, y); }
...
function _kneeSetInput(id, val){
    // Intentionally a no-op for the value boxes: like every other setting on the site, the
    // input stays blank and the current value is shown only in the _echo span in the label.
}
function fetchKneeLearnState(){
    return fetch(buildURL('/kneeLearnState')).then(r=>r.json()).then(j=>{
        ...
        const tcCb = document.getElementById('kneeTempComp_checkbox');
        if (tcCb) tcCb.checked = tc;
        _kneeSetEcho('kneeTempRefF_echo',    f1(toDisplayTemp(j.tempRefF)));    _kneeSetInput('kneeTempRefF_input',    f1(toDisplayTemp(j.tempRefF)));
        ...
        _kneeSetEcho('kneeTempTolF_echo',    f1(toDisplayTempDelta(j.tempTolF)));    _kneeSetInput('kneeTempTolF_input',    f1(toDisplayTempDelta(j.tempTolF)));
...
const ALT_STATE_COLOR = ['#2e9e4f','#3a7bd5','#888','#888'];   // measured green / estimated blue (spec §3); 2/3 grey (no number)
const ALT_RING_COLOR  = '#f0ad4e';                             // orange ring outlining a FULL steady-run dot (overlay, spec §3)
...
//    since this dashboard session began, uPlot-styled like the Plots tab. Marked by confidence state:
//    MEASURED = filled green dot, ESTIMATED = filled blue dot; a point that is also a FULL steady run
...
function buildAltSessPlot(){
  ...
  const opts = {
    ...
    height: 200,
    // Top inset reserves a clear strip for the on-plot legend + auto control (both at top:6px),
    // so the plotting area / data line never reaches up under them.
    padding: [34, null, null, null],
    series: [
      { label: 'Time' },
    ...
    hooks: { draw: [u => {
      ...
      // Measured = filled green dot; Estimated = filled blue dot (spec §3). A point that was also a
      // FULL steady run (altSessRing[i]) gets an orange ring drawn around it — an overlay that does not
      // change the dot color. Loop once: fill the right-colored dot, then ring full-steady points.
      for (let i = 0; i < altSessT.length; i++) {
        const meas = altSessMeas[i], est = altSessEst[i];
        if (meas == null && est == null) continue;
        const isMeas = meas != null;
        const x = u.valToPos(altSessT[i], 'x', true);
        const y = u.valToPos(isMeas ? meas : est, 'y', true);
        ctx.beginPath(); ctx.arc(x, y, 2.7, 0, 6.2832);
        ctx.fillStyle = isMeas ? ALT_STATE_COLOR[0] : ALT_STATE_COLOR[1]; ctx.fill();
        if (altSessRing[i] === 1) {     // full steady run → orange ring outlining the dot
          ctx.beginPath(); ctx.arc(x, y, 4.8, 0, 6.2832);
  ...
  leg.innerHTML =
    ...
    + '<span style="display:flex;align-items:center;gap:5px;white-space:nowrap;">'
    + '<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:' + ALT_STATE_COLOR[1] + ';"></span>'
    + '<b style="font-weight:600;">Estimated</b></span>'
    + '<span style="width:1px;height:13px;background:var(--border-light,#e3e6e8);"></span>'
    + '<span style="display:flex;align-items:center;gap:5px;white-space:nowrap;">'
    + '<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:transparent;border:1.6px solid ' + ALT_RING_COLOR + ';box-sizing:border-box;"></span>'
    + '<b style="font-weight:600;">Steady run</b></span>';
  el.appendChild(leg);
...
let altSessLastPushMs = 0;
function altSessionPush(){
  // One point every 5 s (spec §3). The SSE feed is ~1 Hz; throttle here so the plot is 5 s-cadenced.
  const nowMs = Date.now();
  if (altSessLastPushMs && (nowMs - altSessLastPushMs) < 5000) return;
  altSessLastPushMs = nowMs;
  const st = altLive.state|0;
  // Plot a dot only when the SESSION steady gate is met AND the point is graded (Measured/Estimated).
  // Below the gate, or no reference, or not running → grey gap (no dot).
  const graded = altLive.valid && altLive.sessSteady>=1 && (st===0 || st===1) && altLive.pct>0;
  altSessT.push(nowMs/1000);
...
function initializeEventSource() {
    ...
    try {
        ...
        source.addEventListener('AltSettings', function (e) {
            ...
            const v = e.data.split(',').map(Number);
            const ALT_NOECHO = { altPaused: 1 };   // driven by the Pause/Continue + Reference Source buttons, no echo span
            altSchema.settings.forEach((k, i) => {
                altSettings[k] = v[i];
                if (ALT_NOECHO[k]) return;
                const el = document.getElementById(k + '_echo');
                if (el) el.textContent = (k === 'altThermDegF')   // temperature band — follows C/F toggle (delta)
...
// Field % (right-hand "pct") axis ranges — click-to-edit, per live plot. Default 0-100,
// reset by the autoscale lock. Local-only (no firmware param like the Ymin/Ymax pairs).
let pctMin1 = 0, pctMax1 = 100; // Current plot Field %
let pctMin2 = 0, pctMax2 = 100; // Voltage plot Field %
let pctMin3 = 0, pctMax3 = 100; // RPM plot Field %
let pctMin4 = 0, pctMax4 = 100; // Temperature plot Field %
...
function updatePlotConfiguration(data) {
    ...
    if (data.webgaugesinterval !== cachedWebgaugesInterval) {
        ...
        // Tuning plots' 5-min buffers are interval-sized too — re-grid them at the new point spacing.
    ...
    if (pidTuningTimeChanged) {
        ...
        // Re-window in place — just re-run the x-range fn. The 5-min buffer is window-independent, so an
        // xTime echo (incl. the 60s heartbeat) never reallocates or loses history; it only slides the view.
        if (pidTuningPlot && pidTuningData) pidTuningPlot.setData(pidTuningData, true);
        updatePIDXButtons();

        // CV plot uses xTime as its fallback window when cvXTime is null
        if (cvTuningPlot && cvTuningData && cvXTime === null) {
            cvTuningPlot.setData(cvTuningData, true);
        }
...
function updateAllEchosOptimized(data) {
    ...
    const specialDisplays = [
        ...
        {
            // Persisted plant time constant (ms) from the Plant Delay sweep fit. Adopt it into the
            // session variable ONLY when this session has no fresher fit (sysidFitTauMs<=0), so a
            // brand-new in-session sweep is never clobbered by a slightly-stale echo during the
            // commit round-trip. Drives the readout beside the Compute button.
            key: 'systemIDPlantTauMs',
            id: 'faPlantTau_readout',
            transform: v => v,
            special: (el, value) => {
                if (sysidFitTauMs <= 0 && value > 0) sysidFitTauMs = value;
                updatePlantTauReadout(sysidFitTauMs);
            }
        },
        {
            key: 'AlarmLatchState',
...
async function fetchAndPopulateVesselInfo() {
    try {
        ...
        if (!response.ok) {
            ...
            // Dev note: comment out the line below to allow local file previewing without vessel_info.json
...
async function handleVesselInfoSave(event) {
    ...
    // If the user changed system voltage, warn before saving — /saveVesselInfo rescales the whole
...
function renderSystemIDLog(data) {
    ...
    if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="14" style="padding:12px;text-align:center;color:var(--text-muted);">No plant-delay runs logged yet.</td></tr>';
        return;
    }
...
let _sysidSweepPollTimer = null;
let _tuningSweepPollTimer = null;

function fetchSysidSweepLog() {
    fetch(buildURL('/sysidsweeplog'))
        .then(r => r.ok ? r.json() : null)
        .then(d => { if (d) renderSysidSweepLog(d); })
        .catch(() => {});
}
function fetchTuningSweepLog() {
    fetch(buildURL('/tuningsweeplog'))
        .then(r => r.ok ? r.json() : null)
        .then(d => { if (d) renderTuningSweepLog(d); })
        .catch(() => {});
}

function sweepCurveRowHTML(pts, cols, gainLabel) {
    let t = '<table style="width:auto;border-collapse:collapse;font-size:11px;margin:4px 0 6px 18px;">' +
        '<thead><tr style="color:var(--text-muted);text-align:left;">' +
        '<th style="padding:2px 10px;">Freq (Hz)</th><th style="padding:2px 10px;">' + gainLabel + '</th>' +
        '<th style="padding:2px 10px;">Phase lag (°)</th></tr></thead><tbody>';
    (pts || []).forEach(p => {
        t += '<tr><td style="padding:2px 10px;">' + p.f.toFixed(2) + '</td>' +
             '<td style="padding:2px 10px;">' + p.g.toFixed(3) + '</td>' +
             '<td style="padding:2px 10px;">' + p.ph.toFixed(0) + '</td></tr>';
    });
    t += '</tbody></table>';
    return '<tr class="sweep-curve-row" style="display:none;"><td colspan="' + cols + '" style="padding:0;">' + t + '</td></tr>';
}

function toggleSweepCurve(rowEl) {
    const next = rowEl.nextElementSibling;
    if (next && next.classList.contains('sweep-curve-row')) {
        next.style.display = (next.style.display === 'none' || !next.style.display) ? '' : 'none';
    }
}

function renderSysidSweepLog(data) {
    const tbody = document.getElementById('sysidSweepLogBody');
    if (!tbody) return;
    const recs = data.rec || [];
    if (!recs.length) {
        tbody.innerHTML = '<tr><td colspan="12" style="padding:12px;text-align:center;color:var(--text-muted);">No sweeps logged yet.</td></tr>';
        return;
    }
    const fmt = (v, d) => (v == null || !isFinite(v) || v < 0) ? '—' : v.toFixed(d);
    tbody.innerHTML = recs.map(r => {
        const ro = (r.ro != null && r.ro >= 0) ? r.ro : -1;
        const roColor = ro < 0 ? '#ef4444' : (ro >= 5 ? '#22c55e' : ro >= 2 ? '#eab308' : '#ef4444');
        const summary = `<tr class="sweep-summary-row" style="cursor:pointer;" onclick="toggleSweepCurve(this)">
            <td style="padding:2px 6px;">${r.n}</td>
            <td style="padding:2px 6px;color:${roColor};font-weight:bold;">${fmt(ro, 1)}</td>
            <td style="padding:2px 6px;">${fmt(r.dc, 3)}</td>
            <td style="padding:2px 6px;">${fmt(r.wp, 0)}</td>
            <td style="padding:2px 6px;">${fmt(r.amp, 1)}</td>
        ...
        return summary + sweepCurveRowHTML(r.pts, 12, 'Gain (A/%)');
    }).join('');
}

// Format a tuning-record wall-clock timestamp (firmware epoch seconds, 0 = clock not synced
...
function renderTuningSweepLog(data) {
    const tbody = document.getElementById('tuningSweepLogBody');
    if (!tbody) return;
    const recs = data.rec || [];
    if (!recs.length) {
        tbody.innerHTML = '<tr><td colspan="18" style="padding:12px;text-align:center;color:var(--text-muted);">No sweeps logged yet.</td></tr>';
        return;
    }
    const curKp = parseFloat(document.getElementById('PidKp_echo')?.textContent);
    const curKi = parseFloat(document.getElementById('PidKi_echo')?.textContent);
    ...
    const fmt = (v, d) => (v == null || !isFinite(v) || v < 0) ? '—' : v.toFixed(d);
    tbody.innerHTML = recs.map(r => {
        const bw = (r.bw != null && r.bw >= 0) ? r.bw : -1;
        const bwColor = bw < 0 ? '#ef4444' : (bw >= 5 ? '#22c55e' : bw >= 2 ? '#eab308' : '#ef4444');
        const match = (Math.abs(r.kp - curKp) < 1e-4 && Math.abs(r.ki - curKi) < 1e-4 && Math.abs(r.kd - curKd) < 1e-5);
        // Coherence + clip flag mark how much to trust the row's gain/phase numbers.
        const coh = (r.coh != null) ? r.coh : 1;
        const cohColor = coh >= 0.9 ? '#22c55e' : coh >= 0.7 ? '#eab308' : '#ef4444';
        const clipColor = r.clip ? '#ef4444' : 'inherit';
        const summary = `<tr class="sweep-summary-row" style="cursor:pointer;${match ? 'background:rgba(74,158,255,0.15);' : ''}" onclick="toggleSweepCurve(this)">
            <td style="padding:2px 6px;">${r.n}</td>
            <td style="padding:2px 6px;color:${bwColor};font-weight:bold;">${fmt(bw, 1)}</td>
            <td style="padding:2px 6px;">${fmt(r.pg, 2)}</td>
            <td style="padding:2px 6px;">${fmt(r.wp, 0)}</td>
            <td style="padding:2px 6px;">${fmt(r.kp, 3)}</td>
            <td style="padding:2px 6px;">${fmt(r.ki, 3)}</td>
            <td style="padding:2px 6px;">${fmt(r.kd, 4)}</td>
            <td style="padding:2px 6px;">${fmt(r.f0, 1)}–${fmt(r.f1, 1)}</td>
            <td style="padding:2px 6px;">${fmt(r.amp, 2)}</td>
            <td style="padding:2px 6px;">${fmt(r.base, 1)}</td>
            <td style="padding:2px 6px;">${fmt(r.bv, 2)}</td>
            <td style="padding:2px 6px;">${fmt(r.rpm, 0)}</td>
        ...
        return summary + sweepCurveRowHTML(r.pts, 18, 'Gain (rel)');
    }).join('');
}
...
document.addEventListener('DOMContentLoaded', () => {
    const s1 = document.getElementById('sysidSweepLogSection');
    if (s1) s1.addEventListener('toggle', e => {
        if (e.target.open) { fetchSysidSweepLog(); _sysidSweepPollTimer = setInterval(fetchSysidSweepLog, 10000); }
        else { clearInterval(_sysidSweepPollTimer); _sysidSweepPollTimer = null; }
    });
    const s2 = document.getElementById('tuningSweepLogSection');
    if (s2) s2.addEventListener('toggle', e => {
        if (e.target.open) { fetchTuningSweepLog(); _tuningSweepPollTimer = setInterval(fetchTuningSweepLog, 10000); }
        else { clearInterval(_tuningSweepPollTimer); _tuningSweepPollTimer = null; }
    });
});
...
function initCVTuningDataStructures() {
    const intervalMs = window._lastKnownInterval || 200;
    // Buffer ALWAYS holds the full 5 min (LIVE_BUFFER_SEC), independent of the selected X window —
    // so switching to 5M always has history and a window change never reallocates or loses data.
    // The x-scale range fn (in initCVTuningPlot) draws only the selected window; sizing here is by
    // interval alone. Mirrors the main live plots (reinitializePlotsWithNewTiming).
...
function initCVTuningPlot() {
    ...
    const opts = {
        ...
        scales: {
            ...
            // x: fixed [-width, 0] with the right edge pinned at 0 — NOT data-driven [x[0], x[last]].
            // applyInterp() slides x[last] left by elapsedSec each frame and snaps it back at every
            // packet arrival; feeding that sawtooth into the range makes the whole axis (ticks + data)
            // shiver left/right 5x/sec. Width is the SELECTED window (cvXTime, else xTime), NOT the
            // buffer span — the 5-min buffer holds the history, this just draws the visible slice, so a
            // window change re-ranges in place via plain setData with no realloc.
...
function initCurrentTempPlot() {
    ...
    const opts = {
        ...
        scales: useTimestamps ? {
            ...
            pct: { auto: false, range: () => [pctMin1, pctMax1] }     // fn so click-to-edit Field % range survives redraws
        } : {
    ...
    attachYAxisEdit(currentTempPlot, [{
        ...
        // Field % (right axis) — local-only, clear + Enter returns to 0-100
        scale: 'pct', decimals: 0,
        apply: (mn, mx) => { pctMin1 = mn; pctMax1 = mx; currentTempPlot.setScale('pct', { min: mn, max: mx }); },
        auto: () => { pctMin1 = 0; pctMax1 = 100; currentTempPlot.setScale('pct', { min: 0, max: 100 }); }
    }]);
}
...
function initVoltagePlot() {
    ...
    const opts = {
        ...
        scales: useTimestamps ? {
            ...
            pct: { auto: false, range: () => [pctMin2, pctMax2] }     // fn so click-to-edit Field % range survives redraws
        } : {
            x: { time: false, range: (u, dMin, dMax) => [-liveWindowSec, 0] },
            voltage: { auto: false, range: () => [Ymin2 / 100, Ymax2 / 100] },   // fn, not array: re-read each redraw so Y edits survive setData re-ranging
            pct: { auto: false, range: () => [pctMin2, pctMax2] }     // fn so click-to-edit Field % range survives redraws
    ...
    attachYAxisEdit(voltagePlot, [{
        ...
        // Field % (right axis) — local-only, clear + Enter returns to 0-100
        scale: 'pct', decimals: 0,
        apply: (mn, mx) => { pctMin2 = mn; pctMax2 = mx; voltagePlot.setScale('pct', { min: mn, max: mx }); },
        auto: () => { pctMin2 = 0; pctMax2 = 100; voltagePlot.setScale('pct', { min: 0, max: 100 }); }
    }]);
}
...
function initRPMPlot() {
    ...
    const opts = {
        ...
        scales: useTimestamps ? {
            ...
            pct: { auto: false, range: () => [pctMin3, pctMax3] }     // fn so click-to-edit Field % range survives redraws
        } : {
            x: { time: false, range: (u, dMin, dMax) => [-liveWindowSec, 0] },
            rpm: { auto: false, range: () => [Ymin3, Ymax3] },   // fn, not array: re-read each redraw so Y edits survive setData re-ranging
            pct: { auto: false, range: () => [pctMin3, pctMax3] }     // fn so click-to-edit Field % range survives redraws
    ...
    attachYAxisEdit(rpmPlot, [{
        ...
        // Field % (right axis) — local-only, clear + Enter returns to 0-100
        scale: 'pct', decimals: 0,
        apply: (mn, mx) => { pctMin3 = mn; pctMax3 = mx; rpmPlot.setScale('pct', { min: mn, max: mx }); },
        auto: () => { pctMin3 = 0; pctMax3 = 100; rpmPlot.setScale('pct', { min: 0, max: 100 }); }
    }]);
}
...
function initTemperaturePlot() {
    ...
    attachYAxisEdit(temperaturePlot, [{
        ...
        // Field % (right axis) — local-only, clear + Enter returns to 0-100
        scale: 'pct', decimals: 0,
        apply: (mn, mx) => { pctMin4 = mn; pctMax4 = mx; temperaturePlot.setScale('pct', { min: mn, max: mx }); },
        auto: () => { pctMin4 = 0; pctMax4 = 100; temperaturePlot.setScale('pct', { min: 0, max: 100 }); }
    }]);
}
...
function updateTogglesFromData(data) {
    try {
        ...
        // (skip while the user is actively clicking one of its buttons).
        if (data.systemIDTestType !== undefined) {
            const af = document.activeElement;
            if (!(af && /^systemIDTestType_btn/.test(af.id || ''))) syncSysidTestTypeUI(data.systemIDTestType);
        }
        // Tuning waveform selector (segmented): reflect saved value + show/hide period/sine rows.
        if (data.tuningWaveform !== undefined) {
            const af = document.activeElement;
...
function confirmVoltageRescale(oldV, newV) {
    ...
    const line = (label, echoId) => {
        ...
        return '  ' + label + ': ' + fmt(o) + ' → ' + fmt(o * ratio) + ' V';
    };
    // Field-duty knobs scale by the INVERSE ratio (oldV/newV) so the field CURRENT they govern stays
    // constant across banks; unit is %/s (or %), not volts.
    const dline = (label, echoId, unit) => {
        const o = getEchoNumber(echoId);
        return '  ' + label + ': ' + fmt(o) + ' → ' + fmt(o / ratio) + ' ' + unit;
    };
    const newBulk = getEchoNumber('BulkVoltage_echo') * ratio;
    const hardV = Number.isFinite(newBulk) ? newBulk + 0.3 : NaN;
    const msg = 'Changing system voltage from ' + oldV + 'V to ' + newV + 'V (×'
        ...
        + line('Bulk', 'BulkVoltage_echo') + '\n'
        + line('Absorption', 'AbsorptionVoltage_echo') + '\n'
        + line('Float', 'FloatVoltage_echo') + '\n'
        + line('Rebulk', 'RebulkVoltage_echo') + '\n'
        + line('Target', 'TargetVoltageSetpoint_echo') + '\n'
...
window.addEventListener("load", function () {
    ...
    window.attachStreamListeners = function (source) {
        ...
        const handleCSVData = function (e) {
            ...
            // CV plant-fit (commissioning step 8): capture {t, battery voltage, measured amps} while a fit runs.
        ...
        source.addEventListener('CSVData2', function (e) {
            ...
            const updateFields = (fieldArray) => {
                for (const [elementId, key] of fieldArray) {
                    ...
                    // CSV2 build/send cost: firmware sends µs; display in ms
                    else if (key === "csv2BuildLast" || key === "csv2BuildWorst" || key === "csv2SendLast" || key === "csv2SendWorst") {
                        newTextContent = (value / 1000).toFixed(2);
                    }
                    ...
                    // branch below or toDisplayTemp turns it into -73 in °C mode). Covers the live
                    // reading plus the Session/Lifetime worst-value readouts.
                    else if (["temperatureThermistor", "MaxTemperatureThermistor", "MaxTemperatureThermistor_AllTime"].includes(key) && num === -99) {
                        newTextContent = "—";
                    }
            ...
            const otherFields = [
                ...
                ["csv2SendWorst_ID", "csv2SendWorst"],
                ...
                ["httpsUpload_win_ID", "httpsUpload_win"],
                ["httpsUpload_ses_ID", "httpsUpload_ses"],
            ...
            // e.g. tuningStopTest/updateTestActivePanel) instead of the undefined data.* members.
            const tuningModeActive = parseInt(getField("TuningMode") ?? 0) === 1;
            const cvTuningModeActive = parseInt(getField("CVTuningMode") ?? 0) === 1;
            ...
            _testActiveCSV1 = tuningModeActive ? 'curr' : cvTuningModeActive ? 'cv' : null;
...
// uPlot sizes itself to clientWidth at construction. All six live charts are built once at load
// (initCurrentTempPlot()... near startInterpLoop()) while their tab is display:none, i.e. at width 0.
// Each has a ResizeObserver that fixes this when the element gains size, but that's debounced, so the
...
function resizeLivePlotsOnShow() {
    const fix = (plot, elId, height) => {
        if (!plot) return;
        const el = document.getElementById(elId);
        if (el && el.clientWidth > 0) plot.setSize({ width: el.clientWidth, height });
    };
    fix(currentTempPlot, 'current-temp-plot', 300);
    fix(voltagePlot,     'voltage-plot',      300);
    fix(rpmPlot,         'rpm-plot',          300);
...
function showSubTab(parentTab, subTabName, evt = null) {
    ...
    if (parentTab === 'plots' || parentTab === 'tuning') resizeLivePlotsOnShow();
...
function initPidTuningDataStructures() {
    ...
    // so switching to 5M always has history and a window change never reallocates or loses data. The
    // x-scale range fn (in initPidTuningPlot) draws only the selected window. Mirrors the main live plots.
    const maxPoints = Math.ceil(LIVE_BUFFER_SEC * 1000 / intervalMs);

    if (maxPoints <= 0 || maxPoints > 10000 || isNaN(maxPoints)) {
        diagError(`Invalid maxPoints: ${maxPoints} (interval=${intervalMs}, buffer=${LIVE_BUFFER_SEC}s)`);
        return;
    }
    ...
    const opts = {
        ...
        scales: {
            x: {
                ...
                // Fixed [-width, 0] with the right edge pinned at 0, NOT data-driven [x[0], x[last]]:
                // applyInterp() slides x[last] left then snaps it back each packet, which a data-driven
                // range renders as an x-axis tick shiver. Width derived from the buffer tracks the window.
                range: () => { const w = (xTime && xTime > 0) ? xTime : 60; return [-w, 0]; }   // selected window, NOT buffer span — 5-min buffer holds history, this draws the visible slice
            },
            amps: {
...
function setPIDXTime(val) {
    ...
    // Re-window in place: buffer already holds 5 min, just re-run the x-range fn — no realloc, no history loss.
    if (pidTuningPlot && pidTuningData) pidTuningPlot.setData(pidTuningData, true);
    updatePIDXButtons();
...
// Paint the pause/resume toggle's visual state (dot color + label + the dataset
// flag toggleLogs reads). Pure render — no reconciliation. Its dot carries the
// state (pulsing green = recording, amber = paused), the label names the action.
function renderLoggingToggle(on) {
    g_loggingActive = on ? 1 : 0;  // remembered for the log-relay capture-state stamp
    ...
    toggle.dataset.recording = on ? '1' : '0';   // so toggleLogs() knows which call to make
}

// Reflect live logging state (CSV2 slot 439). Honors a pending optimistic toggle
// so a STALE echo (loggingActive only streams on CSV2's ~5s cadence) cannot snap
// the button back to its pre-click state during the round-trip. Same pendingToggles
// + deadline guard as faUpdatePauseBtn / updateSegToggle.
function updateLoggingStatus(loggingActive) {
    const on = Number(loggingActive) === 1;
    const pending = pendingToggles.get('loggingActive');
    if (pending) {
        if (pending.deadlineMs === undefined) pending.deadlineMs = Date.now() + 8000;  // covers a 5s CSV2 cycle + jitter
        if ((on ? 1 : 0) !== pending.desiredValue) {
            if (Date.now() <= pending.deadlineMs) return;   // stale echo — ignore until the device catches up
            pendingToggles.delete('loggingActive');         // gave up waiting — accept the device value
        } else {
            pendingToggles.delete('loggingActive');         // device confirmed the click
        }
    }
    renderLoggingToggle(on);
}

// Single button toggles pause/resume. Flips the button IMMEDIATELY (optimistic)
// instead of waiting for the ~5s CSV2 echo — the old behavior left the button
// looking dead for seconds, so a second "did it work?" click flip-flopped the
// state and could leave logging silently paused for a whole session.
function toggleLogs() {
    const toggle = document.getElementById('logToggleBtn');
    if (!toggle) return;
    const goRecording = toggle.dataset.recording !== '1';   // target = opposite of current
    pendingToggles.set('loggingActive', { desiredValue: goRecording ? 1 : 0, baseRev: lastSeenRev });
    renderLoggingToggle(goRecording);                       // instant feedback — no CSV2 wait
    (goRecording ? startLogs : stopLogs)();                 // POST; CSV2 echo reaffirms via updateLoggingStatus
}
...
function makePanelDraggable(panel, handle) {
    ...
    function onDown(e) {
        ...
        panel.style.transform = 'none';   // drop any centering transform so left/top are absolute
        document.addEventListener('mousemove', onMove);
        document.addEventListener('mouseup',   onUp);
        document.addEventListener('touchmove', onMove, { passive: false });
        document.addEventListener('touchend',  onUp);
...
function updateTestActivePanel() {
    ...
    const cxOv = document.getElementById('commission-modal-overlay');
    if (cxOv && cxOv.style.display === 'block') {
        overlay.style.display = 'none';
...
// Hidden input systemIDTestType_sel mirrors the value so getField()/the modal can read it.
function syncSysidTestTypeUI(v) {
    v = parseInt(v);
    const sel = document.getElementById('systemIDTestType_sel');
    if (sel) sel.value = String(v);
    const b0 = document.getElementById('systemIDTestType_btn0');
    const b1 = document.getElementById('systemIDTestType_btn1');
    if (b0) b0.classList.toggle('on', v === 0);
    if (b1) b1.classList.toggle('on', v === 1);
    sysidToggleSineParams(v);
}
...
function syncTuningWaveformUI(v) {
    v = parseInt(v);
    const sel = document.getElementById('tuningWaveform_sel');
    if (sel) sel.value = String(v);
    for (let i = 0; i < 3; i++) {
        const b = document.getElementById('tuningWaveform_btn' + i);
        if (b) b.classList.toggle('on', v === i);
    }
    tuningToggleWaveformUI(v);
}
...
function renderTuningBode(data) {
    ...
    if (document.getElementById('tuningSweepLogSection')?.open) fetchTuningSweepLog();
    ...
    // Log-frequency interpolation of the −3 dB crossing (points are log-spaced) — matches the
    // open-loop FOPDT fit and the firmware-logged value so all three readouts agree.
    let f3 = null;
    for (let i = 1; i < pts.length; i++) {
        ...
        if (relPrev >= 0.708 && relCur < 0.708) {
            ...
            const t = (0.708 - relPrev) / (relCur - relPrev);
...
// "Biggest actionable disturbance" — worst tone the FIELD can react to, read from the resonance
// /ripple matrix (/famatrix.csv). Actionable band is [4 Hz, actuator BW], where actuator BW =
// Readout beside the (usually hidden) Compute button: shows the fitted plant time constant and the
// actuator bandwidth it implies (1/2πτ), or a muted "run the sweep" prompt when nothing is fitted yet.
// Fed by the persisted CSV3 echo (survives reboots) and refreshed instantly on a fresh fit.
// Also renders the static "biggest actionable disturbance" conclusion into #faActionableResult: on a
// normal slow-field install the actuator band sits below the 4 Hz ripple floor, so no measured tone is
// ever chaseable — a fixed fact, not worth a button. The interactive Compute scan (#faComputeBtn) is
// revealed only if a fit ever puts the band above 4 Hz (τ < ~40 ms); otherwise the button stays hidden.
function updatePlantTauReadout(tauMs) {
    const el = document.getElementById('faPlantTau_readout');
    const FLOOR_HZ = 4.0;   // matrix FFT peak-search floor (FA_MIN_TONE_HZ)
    if (el) {
        if (tauMs > 0) {
            const bwHz = 1000 / (2 * Math.PI * tauMs);
            el.innerHTML = 'Plant &tau;: <strong>' + Math.round(tauMs) + ' ms</strong> &middot; actuator band &le; ' +
                bwHz.toFixed(1) + ' Hz';
            el.style.color = '';
        } else {
            el.innerHTML = 'Plant &tau;: not fitted yet — run a Plant Delay sine sweep in Commissioning';
            el.style.color = 'var(--text-muted)';
        }
    }
    const res = document.getElementById('faActionableResult');
    const btn = document.getElementById('faComputeBtn');
    if (!res) return;
    if (!(tauMs > 0)) {
        res.innerHTML = 'Run a Plant Delay sine sweep in Commissioning to evaluate this.';
        if (btn) btn.style.display = 'none';
        return;
    }
    const actBwHz = 1000 / (2 * Math.PI * tauMs);
    if (actBwHz <= FLOOR_HZ) {
        res.innerHTML = 'None — the field bandwidth (<strong>' + actBwHz.toFixed(1) + ' Hz</strong>, from plant ' +
            'τ=' + Math.round(tauMs) + ' ms) is below the 4 Hz ripple floor, so the field is too slow to chase any ' +
            'measured ripple tone. Ripple is bounded by ripple-rejection, not reaction speed — nothing to tune here.';
        if (btn) btn.style.display = 'none';
    } else {
        res.innerHTML = 'Plant is fast enough that some tones (band 4–' + actBwHz.toFixed(1) + ' Hz) could be ' +
            'chaseable — press Compute to find the strongest one in the learned map.';
        if (btn) btn.style.display = '';
    }
}

// 1/(2π·τ_plant) from the Plant Delay sine fit (sysidFitTauMs). Tones above it are real but
// un-actionable (field too slow → the iExcess EMA averages them out), so they're excluded.
// Ranked by residual (amp/freq): a low slow tone survives the EMA far better than a high one.
// On a slow-field install the band can fall below 4 Hz → nothing is actionable (the common case).
// See iExcess_Redesign_Spec.md §11.
function computeActionableDisturbance() {
    const el = document.getElementById('faActionableResult');
    if (!el) return;
    const tauMs = sysidFitTauMs;   // fitted plant τ (ms); 0 until a Plant Delay sweep is fit
    if (!(tauMs > 0)) {
        el.textContent = 'Run a Plant Delay sine sweep first — the actionable band edge comes from the fitted plant time constant.';
        return;
    }
    const actBwHz  = 1000 / (2 * Math.PI * tauMs);   // actuator bandwidth, Hz
    const FLOOR_HZ = 4.0;                            // matrix FFT peak-search floor (FA_MIN_TONE_HZ)
    if (actBwHz <= FLOOR_HZ) {
        el.innerHTML = 'No actionable disturbance — actuator bandwidth (<strong>' + actBwHz.toFixed(1) +
            ' Hz</strong>, from plant τ=' + Math.round(tauMs) + ' ms) is below the 4 Hz measurement floor. ' +
            'Every measurable tone is faster than the field can chase, so all are averaged out. ' +
            'iExcess τ is bounded by ripple-rejection, not reaction speed.';
        return;
    }
    el.textContent = 'Computing…';
    const tauIeMs = parseFloat(getField('IExcessTau_echo'));   // detector EMA τ, for the residual figure
    const FA_RPM_BIN_W = 50;   // mirror of the firmware matrix RPM bin width
    fetch(buildURL('/famatrix.csv')).then(r => r.text()).then(txt => {
        ...
        for (let i = 1; i < lines.length; i++) {   // skip header row
            ...
            const rpmCtr = parseFloat(c[0]) + FA_RPM_BIN_W / 2;
            for (let s = 0; s < 6; s++) {           // columns: f1,a1,n1 at 4,5,6; stride 3
                const f = parseFloat(c[4 + s * 3]);
                const a = parseFloat(c[5 + s * 3]);
                const n = parseFloat(c[6 + s * 3]);
                if (!(n > 0) || !(a > 0) || !(f >= FLOOR_HZ) || f > actBwHz) continue;
                const residIdx = a / f;             // ∝ post-EMA residual (τ constant across the comparison)
                if (!best || residIdx > best.residIdx) best = { f, a, rpm: rpmCtr, residIdx };
            }
        }
        if (!best) {
            el.innerHTML = 'No actionable tone in 4–' + actBwHz.toFixed(1) +
                ' Hz across the learned map (plant τ=' + Math.round(tauMs) + ' ms). All measurable disturbances ' +
                'are above the actuator bandwidth, so the field can’t chase them and the EMA averages them out.';
            return;
        }
        let resStr = '';
        if (tauIeMs > 0) {
            const resid = best.a / (2 * Math.PI * best.f * (tauIeMs / 1000));   // post-EMA residual, A (spec §11)
            resStr = ' — post-EMA residual ≈ <strong>' + resid.toFixed(2) + ' A</strong> at the current iExcess τ (' +
                Math.round(tauIeMs) + ' ms); set the detector floor above this.';
        }
        el.innerHTML = 'Biggest actionable disturbance: <strong>' + best.a.toFixed(1) + ' A pk-pk @ ' +
            best.f.toFixed(1) + ' Hz</strong>, ' + Math.round(best.rpm) + ' RPM (band 4–' + actBwHz.toFixed(1) +
            ' Hz, plant τ=' + Math.round(tauMs) + ' ms)' + resStr;
    }).catch(e => { el.textContent = 'Matrix fetch failed: ' + e; });
}
...
// ============================================================================
// Auto-commissioning orchestrator (spec §12). One-button guided setup of the
// current-control + over-current chain. Drives existing /get handlers and the
...
// ============================================================================
const CX_PHASES = ['Prep', 'Field curve', 'Min% floor', 'Plant fit', 'Verify', 'Disturbances', 'Thresholds', 'CV plant fit'];
...
function cxApplyDeps() {
    ...
    while (changed) {
        ...
        for (const k in CX_DEPENDENTS) {
            if (cxPlan[+k]) for (const d of CX_DEPENDENTS[k]) if (!cxPlan[d]) { cxPlan[d] = true; changed = true; }
        }
    }
}
...
// the persisted done-mask (cxLastMask), NOT from the firmware state flag. Optional stages are 1..N
// (Prep always runs and is ignored here). Returns label + help + style + whether to revert.
//   nothing ticked            → disabled.
//   nothing done yet          → "Start" (clean first pass over the ticked subset).
//   every optional step ticked AND something already done → "Clear and restart": a wholesale redo,
//     so revert to the pre-commissioning snapshot and run the whole walkthrough again.
//   otherwise (a partial pass over not-yet-done, and/or a TARGETED redo of a few done steps that
//     force-pulls only their downstream) → "Continue": keep prior work + the snapshot, run the
//     ticked subset. This is the case that must NOT revert, or the "redo forces downstream" coupling
...
// Shared "Next →"/"Finish" button for the per-phase render functions — only emitted once the step's
// action is done, so it's always clickable (no disabled-greyed Next cluttering earlier states).
// label shows the next selected stage, or "Finish" when this is the last one in the plan.
// markDone=false skips marking this stage complete (passed to cxAdvance); primary=false renders it
// as the secondary action (e.g. an explicit "skip" after a failed step).
function cxNextBtn(markDone = true, primary = true) {
...
// The one true protection statement, per phase. Open-loop phases (Field curve 1, Min% floor 2,
// Plant fit 3) and the closed-loop verify (4) are guarded only by the fast OV cut + INA228 backstop;
// the disturbance map (5) and validation (7) run under full normal protection.
...
function cxSafetyText(phase) {
  ...
  // Open-loop / test-current steps (Field curve…Verify, and CV plant fit) run with only the fast
  // over-voltage hard-shutdown live; the rest run under full normal protection.
...
function openCommissionModal() {
    ...
    if (cxHeartbeatTimer) clearInterval(cxHeartbeatTimer);
    cxGet('commissionHeartbeat=1').catch(() => { });
    cxHeartbeatTimer = setInterval(() => { cxGet('commissionHeartbeat=1').catch(() => { }); }, 2000);
}
...
function closeCommissionModal() {
    ...
    if (cxHeartbeatTimer) { clearInterval(cxHeartbeatTimer); cxHeartbeatTimer = null; }
    cxGet('commissionHeartbeat=0').catch(() => { });
    if (cx) {
...
function cxPrepRefresh() {
    ...
    if (isNaN(L.busV) || isNaN(bulk)) { hrEl.innerHTML = '<span style="color:#f0a500;">unknown (no live voltage)</span>'; }
        ...
        const color = head > 1.0 ? '#5a5' : (head > 0.5 ? '#f0a500' : '#f0584f');
        const note = head > 1.0 ? '' : (head > 0.5 ? ' — getting tight' : ' — very little room (OV trip likely)');
        hrEl.innerHTML = '<strong style="color:' + color + ';">' + head.toFixed(2) + ' V below Bulk</strong>'
            + ' (' + L.busV.toFixed(2) + ' / ' + bulk.toFixed(2) + ' V)' + note;
    }
...
function cxRenderField(b) {
    ...
    if (cx.fieldRunning) {
        ...
        body += '<div style="margin:10px 0; padding:8px 10px; background:#222; border-radius:6px;">' +
            ...
            (r.ceilLimited ? '<br><span style="color:#f0a500;">The ramp hit the Max Field % limit before reaching the amp target, so the curve is cut short. If the proposals look low, raise “Max Field (%)” (Settings ▸ Battery) and re-run.</span>' : '') +
...
function cxRenderPlant(b) {
    ...
    if (cx.plantRunning) body += '<p style="color:#4a9eff;">Sine sweep running… hold RPM steady.</p>';
        ...
        else body += '<div style="margin:10px 0; padding:8px 10px; background:#222; border-radius:6px;">' +
            'Plant τ <strong>' + fit.tauMs.toFixed(0) + ' ms</strong>, K ' + fit.K.toFixed(3) + ' A/%, dead time ' + fit.thetaMs.toFixed(0) + ' ms.<br>' +
            ...
            cxPollTimer = setInterval(() => {
                ...
                fetch(buildURL('/sysidbode')).then(r => r.json()).then(j => {
                    ...
                    if (j.ready && !j.active) {
                        ...
                        const pts = j.pts;
                        if (pts.length >= 3) {
                            const fit = sysidFitFOPDT(pts, Math.max(1e-9, pts[0].g));
                            cx.fit = fit;
                            if (fit.ok) {
                            ...
                            if (!fit.ok && !cx._widened) { cx._widened = true; setTimeout(() => cxPlantStart(true), 1500); }
                        } else { cx.fit = { ok: false }; cx.plantRunning = false; commissionRender(); }
                    }
                }).catch(() => { });
            }, 1500);
...
function cxRenderVerify(b) {
    ...
    if (cx.verifyRunning) body += '<p style="color:#4a9eff;">Closed-loop sweep running…</p>';
        ...
        const v = cx.verify;
        body += '<div style="margin:10px 0; padding:8px 10px; background:#222; border-radius:6px;">' +
            'Peak closed-loop gain <strong style="color:' + (v.pass ? '#5a5' : '#f0a500') + ';">' + v.peakGain.toFixed(2) + '</strong>' +
            (v.bwHz > 0 ? ', bandwidth ~' + v.bwHz.toFixed(1) + ' Hz' : '') + '.<br>' +
    ...
    if (!cx.verifyRunning && !cx.verify) body += '<button onclick="cxVerifyStart()" class="btn-primary" style="width:100%;padding:9px;">Run closed-loop verify</button>';
        ...
        // fail → no advance (Detune is primary, Re-run secondary).
        const nx = cxNextSelected(cx.phase);
        const vlabel = (nx === -1) ? 'Finish' : ('Next: ' + CX_PHASES[nx] + ' →');
        const action = cx.verify.pass ? '<button onclick="cxVerifyDone()" class="btn-primary btn-sm">' + vlabel + '</button> ' : '';
        body += '<div style="margin-top:12px;">' + action + '<button onclick="cxVerifyStart()" class="btn-secondary btn-sm">Re-run</button></div>';
    }
    b.innerHTML = body;
}
            ...
            cxPollTimer = setInterval(() => {
                ...
                fetch(buildURL('/tuningbode')).then(r => r.json()).then(j => {
                    if ((j.done && !j.active) || waited > 200) {
                        cxStopPoll();
                        const pts = j.pts || [];
                        let peak = 0; pts.forEach(p2 => { if (p2.g > peak) peak = p2.g; });
                        let bw = -1; const g0 = pts.length ? pts[0].g : 0;
                        for (let i = 1; i < pts.length; i++) { if (pts[i].g < 0.707 * g0) { bw = pts[i].f; break; } }
                        cx.verify = { peakGain: peak, bwHz: bw, pass: (peak > 0 && peak <= 1.15) };
...
// then writes them via the cvPlantK/τ/L handlers and switches CV gains to Auto. No new firmware control
// code — TuningMode already gates G1/G2 (fast-OV stays live) and restores bumplessly on exit.
let cvFitCapture = null;   // {active, samples:[{t,v,i}], t0} while a fit run is recording

function cvFitOnCsv1(data) {
    if (!cvFitCapture || !cvFitCapture.active) return;
    // Capture IBV (INA228 bus voltage) — the SAME signal the CV PI loop controls on
    // (e = voltageTargetSlewed - IBV), NOT BatteryV (ADS1115, a different sensor). Fitting the
    // plant on the loop's actual feedback signal keeps the identified K/τ/L consistent with the
    // loop being tuned. Both are CSV1 fields scaled ×100.
    const v = ('IBV' in data) ? parseFloat(data.IBV) / 100 : NaN;
    const i = ('MeasuredAmps' in data) ? parseFloat(data.MeasuredAmps) / 100 : NaN;
    if (!isFinite(v) || !isFinite(i)) return;
...
function cxRenderCVPlant(b) {
    ...
    if (!cx.cvFitRunning && !f) body += '<button onclick="cxCVPlantStart()" class="btn-primary" style="width:100%;padding:9px;">Run CV plant fit</button>';
        ...
        // until applied, then the advance button. Re-run stays secondary throughout.
        let action;
        if (!f.ok) action = '<button onclick="cxCVPlantStart()" class="btn-primary btn-sm">Re-run</button> ' + cxNextBtn(false, false);
        else if (!cx.cvApplied) action = '<button onclick="cxCVPlantApply()" class="btn-primary btn-sm">Apply</button> <button onclick="cxCVPlantStart()" class="btn-secondary btn-sm">Re-run</button>';
        else action = cxNextBtn(true) + ' <button onclick="cxCVPlantStart()" class="btn-secondary btn-sm">Re-run</button>';
        body += '<div style="margin-top:12px;">' + action + '</div>';
    }
    body += '<p style="font-size:12px;color:#888;margin-top:10px;">After applying, verify on Tuning ▸ Voltage with your own square-wave step (turn CV Tuning Helpers OFF first to read the raw PI).</p>';
    b.innerHTML = body;
}
...
function cxRenderThresh(b) {
    ...
    // One primary action: Apply until applied, then the advance button — which cxNextBtn labels
    // correctly as "Next: CV plant fit →" or "Finish" (no more hardcoded "Finish" that really advances).
    if (cx.thresh && !cx.thresh.err) {
        if (!cx.threshApplied) {
            body += '<div style="margin-top:12px;"><button onclick="cxThreshApply()" class="btn-primary btn-sm">Apply</button></div>';
        } else {
...
function cxComputeThresholds() {
    ...
    const margin = 4;
    // The "floor of the floor" is whatever you've set in the Protections tab (the live
    // IExcessFloorA), not a hardcoded number. Commissioning only ever RAISES it if the
    // disturbance map demands more headroom — it never undercuts a floor you set higher.
    ...
    fetch(buildURL('/famatrix.csv')).then(r => r.text()).then(txt => {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const rpmCtr = parseFloat(c[0]) + 25;
            for (let s = 0; s < 6; s++) {
                const f = parseFloat(c[4 + s * 3]), a = parseFloat(c[5 + s * 3]), n = parseFloat(c[6 + s * 3]);
                if (!(n > 0) || !(a > 0) || !(f >= 4)) continue;
                const idx = a / f;  // residual-weighted: a low slow tone survives the EMA better than a big fast one
...
// phase reached (0..7), and the per-stage done bitmask (bit i = stage i complete).
function renderCommissionStatus(state, phase, mask) {
    ...
    // with work already done). The old separate clear-row is folded into this one button.
    const sb = document.getElementById('commission-start-btn');
    const sh = document.getElementById('commission-start-help');
    const intent = cxStartIntent();
    if (sb) {
...
(function () {
  ...
  function buildLtChart(id, spec) {
    ...
    // Keep the X-axis time ticks lined up across every stacked chart. uPlot reserves a
    // right-hand gutter (~size 50 + labelSize 30 = 80px) for any secondary (side:1) axis.
    // Charts with only a left axis (Currents, Speeds, Angles, Motion) would otherwise
    // extend their plot area that 80px further right and their shared X scale would not
    // match the charts that DO have a secondary axis (Voltage&SOC, Temp&Baro, RPM&Duty).
    // Reserve an identical but invisible right gutter on single-axis charts to match.
    if (!spec.scales.some(sc => sc.side === 1)) {
      axes.push({ scale: spec.scales[0].name, side: 1, label: '',
                  grid: { show: false }, ticks: { show: false },
                  values: (u, ticks) => ticks.map(() => '') });
    }
...
function initFastScopePlot() {
    ...
    asDiv.className = 'autoscale-ctrl';
    asDiv.style.cssText = 'position:absolute;top:6px;right:8px;z-index:10;display:flex;flex-direction:column;align-items:flex-end;gap:2px;font-size:11px;';
    asDiv.innerHTML = '<button id="fastscope-pause-btn" title="Pause — freeze the displayed capture (stops the ~5 s auto-refresh)" style="font-size:10px;padding:0 6px;cursor:pointer;border:1px solid #999;border-radius:2px;background:transparent;line-height:16px;" onclick="fastScopePauseToggle(this)">Pause</button><div style="display:flex;align-items:center;gap:3px;opacity:0.6;"><input type="checkbox" id="autoscale-fastscope-cb" style="cursor:pointer;width:12px;height:12px;margin:0;"><label for="autoscale-fastscope-cb" style="cursor:pointer;user-select:none;">auto</label></div>';
    plotEl.appendChild(asDiv);
    if (fastScopePaused) { const pb = document.getElementById('fastscope-pause-btn'); if (pb) pb.textContent = 'Resume'; }
...
// Pause freezes the displayed capture by halting the ~5 s auto-refresh. The on-chart Pause
// button (top-right of the scope plot) is the only control; press Resume to re-arm the refresh.
...
function fastDiagStartAuto() {
    ...
    fastDiagAutoTimer = setInterval(() => {
        ...
        if (!fastScopePaused) fetchFastScope(true);  // silent: no "Fetching…" flash on the 5 s tick. Pause freezes auto-refresh.
...
const FASTFLIP_PAGE_BYTES = 8020;
const FASTFLIP_NSAMP = 4000;
...
function initFastFlipPlot() {
    ...
    fastFlipPlot = new uPlot({
        ...
        title: "Reference Waveform — 20 kHz",
