...
void SendWifiData() {
  ...
  if (!sentSomething && now - prev_millis5 >= webgaugesinterval && events.count() > 0) {
    ...
    EXT_RAM_ATTR static char payload1[700];  // bumped from 500
  ...
  if (!sentSomething && now - lastpayload2send >= 2000 && events.count() > 0) {
    EXT_RAM_ATTR static char payload2[2400];
    // Format string:
  ...
  if (!sentSomething && now - lastpayload3send >= 2000 && events.count() > 0) {
    EXT_RAM_ATTR static char payload3[1400];
...
SafeInt(ft_uploadBufferedRecords.worstSession / 1000), // 237 — Upload Buffered Records worst session (ms)
  ...
  if (!sentSomething && now - lastTimestampSend >= 3000 && events.count() > 0) {
    ...
    EXT_RAM_ATTR static char timestampPayload[400];
